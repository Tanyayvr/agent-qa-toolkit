...
- Manifest signatures can be produced locally to attest bundle authenticity; for authority- or counsel-facing EU bundles they should be treated as the strict path, not as an afterthought (`scripts/manifest-sign.mjs`).
...
If commercial agreements are required, they are handled contractually and do not require data hosting.
...
2. **One‑time usage / single request**
   - Example: per‑run or per‑team usage agreement.

3. **Enterprise / custom**
   - Pricing and limits defined by contract.

No user accounts or hosted authentication are required for self‑hosted deployments.
