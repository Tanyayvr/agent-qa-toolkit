...
- **Snapshot**: current main HEAD (2026-02-15)
...
- SaaS features (billing, multi-tenant, hosted UI)
...
**Implemented and validated by toolkit tests.**
Evaluator iterates all cases from `readCases()` and pushes one item per case into `items[]`. `data_coverage` is computed. The toolkit test suite validates report shape and counts for known fixtures.

### 3. Manifest closure (all bundle files indexed)
**Hashing present. Manifest entries are validated.**
`source_manifest_sha256` is written to `compare-report.json`. `pvip:verify` validates that manifest entries resolve to existing files and that hashes match. (Directory‑wide closure, i.e. “no extra files”, is not enforced.)

### 4. Portability (no absolute/escape paths in hrefs)
**Detected by quality_flags scan, with opt‑in enforcement.**
`isAbsoluteOrBadHref()` checks for absolute/escape paths. Results are recorded in `quality_flags.portable_paths` and `quality_flags.path_violations`. Enforcement is opt‑in via `--strictPortability` or `pvip:verify:strict`.
...
| Dockerfile | Present (`Dockerfile`, `docker-compose.yml`) |
| Progress bar / ETA | Not found. Runner prints per-case progress via `console.log` in `runner/runner.ts` |
...
| Typed failure classes | `runner/runner.ts`: `FetchFailureClass` type, `RunnerFailureArtifact` type |
| Exit code contract: 0 success, 1 runtime, 2 usage | `runner/runner.ts`: `CliUsageError.exitCode = 2`, `process.exit(1)` in catch. Same pattern in `evaluator/evaluator.ts` |
| Contract versioning in output | `evaluator/evaluator.ts`: `contract_version: 5`, `manifest_version: "v1"` |
...
| What | Where |
|------|-------|
| Response body streamed to disk, not buffered | `runner/runner.ts`: `saveBodyStreamed()` uses `createWriteStream` |
| Write respects Node stream drain protocol | `runner/runner.ts`: `writeStreamOnceDrain()` — awaits `drain` event before writing next chunk |
| Body truncated at maxBodyBytes in stream loop | `runner/runner.ts`: inside `saveBodyStreamed()`, checks `bytesWritten < maxBodyBytes`, slices remainder, sets `truncated = true` |
| Truncation metadata in artifact | `runner/runner.ts`: `body_truncated`, `body_bytes_written`, `max_body_bytes` fields in `RunnerFailureArtifact` |
...
| What | Where |
|------|-------|
| Retry loop with attempt counter | `runner/runner.ts`: `runCase()` — `for (let attempt = 1; attempt <= cfg.retries + 1; ...)` |
| Transient error classification | `runner/runner.ts`: `isTransientFailure()` — timeout/network_error = transient; `httpIsTransient()` — 429/500/502/503/504 |
| Retry only on transient | `runner/runner.ts`: `if (attempt <= cfg.retries && artifact.is_transient === true)` |
| Exponential backoff with jitter | `runner/runner.ts`: `backoffMs()` — `base * 2^(attempt-1)` + 20% random jitter |
| Concurrency worker pool | `runner/runner.ts`: `runWithConcurrency()` — `n` workers drawing from shared index |
| Per-request timeout via AbortController | `runner/runner.ts`: `fetchWithTimeout()` |
...
|---------------|---------------------|----------------|
| `contract_version: 5` | Evaluator output | `evaluator/evaluator.ts` |
| One item per case in `items[]` | Evaluator `main()` loop | `evaluator/evaluator.ts` |
...
| Portability scan (relative hrefs) | Quality flags detection (not enforcement) | `evaluator/evaluator.ts`: `isAbsoluteOrBadHref()`, `computeQualityFlags()` |
| `source_manifest_sha256` | Evaluator output | `evaluator/evaluator.ts`: `sha256Hex()` |
| Embedded manifest index in HTML | HTML report | `evaluator/htmlReport.ts`: `renderHtmlReport()` |
| `data_coverage` summary | Evaluator output | `evaluator/evaluator.ts` |
| `transfer_class` | Evaluator CLI + output | `evaluator/evaluator.ts`: CLI parse + write |
| Bounded body capture | Runner streaming | `runner/runner.ts`: `saveBodyStreamed()` |
| Retry transient only | Runner retry loop | `runner/runner.ts`: `runCase()` |
